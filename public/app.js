...
function renderProspectDetail(item) {
  ...
  $('#detail-panel').innerHTML = `<section class="project-detail"><div class="detail-head"><div><span class="overline">${escapeHtml(item.input.role || item.input.country || 'Expert-led B2B')}</span><h2>${escapeHtml(item.input.companyName)}</h2><p>${escapeHtml(item.input.sourceTitle)}</p></div><span class="status-pill">${escapeHtml(item.status)}</span></div><div class="detail-grid"><div class="info-card"><span>Contact</span><strong>${escapeHtml(item.input.contactName || 'Not supplied')}<br>${escapeHtml(item.input.contactEmail || 'Email required')}</strong></div><div class="info-card"><span>Source</span><strong><a href="${escapeHtml(item.input.sourceUrl)}" target="_blank" rel="noreferrer">Open public source ↗</a></strong></div>${item.sentAt ? `<div class="info-card"><span>Preview opened</span><strong>${item.previewViews ? `${item.previewViews}× · first ${new Date(item.previewFirstViewedAt).toLocaleString()}` : 'Not yet'}</strong></div>` : ''}${item.appliedProjectId ? `<div class="info-card"><span>Application</span><strong>Applied · project ${escapeHtml(item.appliedProjectId.slice(0, 8))}</strong></div>` : ''}</div>${qualification}${item.preview ? `<div class="preview-angle"><span>Campaign angle</span><h3>${escapeHtml(item.preview.campaignAngle)}</h3><p>${escapeHtml(item.preview.whyNow)}</p></div>` : ''}${email}<div class="actions">${actions}</div>${item.error ? `<p style="color:var(--red)">${escapeHtml(item.error)}</p>` : ''}</section>`;
  bindProspectActions();
}
